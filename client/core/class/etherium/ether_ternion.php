...
class ether_wallet{
	...
	public function rpc_wallet_create($data=array()){
		...
		if(!$r['_valid']){
			...
			$r['_err']['_codes']=$_errc;
			return array('_err'=>array('_fields'=>$r['_err']['_fields']));
        }
		
		if(strlen($data['password'])<8){
			return array('_err'=>array('_application.codes.errors.wallets.password_to_low'),'_data'=>false);
		}
		...
		if(!empty($_r['_err'])){
			...
			return array('_err'=>$_errc,'_data'=>false);
		}
		$wallet=$_r['_data'];
		...
		return array('_err'=>false,'_data'=>$_rd);
	}
	...
	public function rpc_wallet_authentication($data=array()){
		...
		if(!$r['_valid']){
			$_errc=$this->_hdv->create_err_codes($r['_err']['_fields'],'_application.codes.errors.wallets');
			$r['_err']['_codes']=$_errc;
			return array('_err'=>array('_fields'=>$r['_err']['_fields']),'_data'=>false);
        }
		$_r=$this->ethereum->personal_unlockAccount($data['wallet'],$data['password']);
		if(!empty($_r['_err'])){
			$_errc=$this->_hdv->create_err_codes($_r['_err'],'_application.codes.errors.wallets');
			return array('_err'=>array(),'_data'=>false);
		}
		session_start();
		...
		return array('_err'=>false,'_data'=>array('wallet'=>$data['wallet'],'balance'=>$balance));
	}
	...
	public function rpc_wallet_buy_tokens($data=array()){		
		...
		$_rd=array('transaction'=>$_r['_data']['transaction'],'wallet'=>$data['wallet']);
		return array('_err'=>false,'_data'=>$_rd);
	}
